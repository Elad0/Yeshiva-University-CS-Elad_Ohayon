...
def inputMC(s):# Function to determine in which column to place piece using monte carlo simulation
    lst=[0]*columns #Create a list to keep track of probability of winning when placing piece in each column
                    #lst[i] is how many games were won when a piece was placed in this column
    for _ in range(100): #Before making a move play 100 games to determine the best one
        random_move= random.randrange(0,columns) # choose a random column to place a piece in
        tmp=cpy(s)
        score_first_move=best_move_random(tmp,random_move) #determine the whether the game was won by calling best_move
        lst[random_move]+=score_first_move 
    max_row=0
    for i in range(len(lst)): #Determine which column had the highest win percentage
        if lst[i] > lst[max_row]:
            max_row=i
    makeMove(s, max_row) #make a move in the column that has the most wins
 
def best_move_random(s,first_move): #Function to determine how a game will end based on a first move
    makeMove(s, first_move) #Place a piece in the given column
    inputRandom(s)                # Human makes a move afterwords
    while not isFinished(s): #Randomly expand the game until it ends using the code from input random Human and MC alternate making moves until finished
        flag = True
        while flag:
            c = random.randrange(0, columns)
            if c < 0 or c >= columns:
                ...
                flag = False
                makeMove(s, c) #Computer makes a move
    return value(s) #Return who won this game
