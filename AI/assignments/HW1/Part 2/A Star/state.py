...
def hdistance2(s): #This will be the Manhattan distance heuristic
    ...
    #print(f"board is {lst}")
    
       
    for i in range(len(lst)):
        column_error= abs(i%n-lst[i]%n)
        row_error= abs(math.floor(i/n) - math.floor(lst[i]/n))
        
        count+=(column_error+row_error)
        
       # print(f"number is: {lst[i]} error of {column_error} on columns and {row_error} on the rows for a total error of {error}")
